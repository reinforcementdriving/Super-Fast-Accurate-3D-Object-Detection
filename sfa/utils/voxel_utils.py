...
# email:
-----------------------------------------------------------------------------------
# Description: utilities for evaluation
# Refer from: https://github.com/qianguih/voxelnet/blob/master/utils/utils.py
"""
...
def lidar_to_bird_view(x, y, factor=1):
    # using the cfgp.INPUT_XXX
    a = (x - cfgp.X_MIN) / cfgp.VOXEL_X_SIZE * factor
    b = (y - cfgp.Y_MIN) / cfgp.VOXEL_Y_SIZE * factor
    a = np.clip(a, a_max=(cfgp.X_MAX - cfgp.X_MIN) / cfgp.VOXEL_X_SIZE * factor, a_min=0)
    b = np.clip(b, a_max=(cfgp.Y_MAX - cfgp.Y_MIN) / cfgp.VOXEL_Y_SIZE * factor, a_min=0)
    return a, b
...
def batch_lidar_to_bird_view(points, factor=1):
    ...
    #   points (N, 2)
    # using the cfgp.INPUT_XXX
    a = (points[:, 0] - cfgp.X_MIN) / cfgp.VOXEL_X_SIZE * factor
    b = (points[:, 1] - cfgp.Y_MIN) / cfgp.VOXEL_Y_SIZE * factor
    a = np.clip(a, a_max=(cfgp.X_MAX - cfgp.X_MIN) / cfgp.VOXEL_X_SIZE * factor, a_min=0)
    b = np.clip(b, a_max=(cfgp.Y_MAX - cfgp.Y_MIN) / cfgp.VOXEL_Y_SIZE * factor, a_min=0)
...
def camera_to_lidar(x, y, z, T_VELO_2_CAM=None, R_RECT_0=None):
    if type(T_VELO_2_CAM) == type(None):
        T_VELO_2_CAM = np.array(cfgp.MATRIX_T_VELO_2_CAM)

    if type(R_RECT_0) == type(None):
        R_RECT_0 = np.array(cfgp.MATRIX_R_RECT_0)
...
def lidar_to_camera_box(boxes, T_VELO_2_CAM=None, R_RECT_0=None):
    ...
    for box in boxes:
        x, y, z, h, w, l, rz = box
        (x, y, z), h, w, l, ry = lidar_to_camera(x, y, z, T_VELO_2_CAM, R_RECT_0), h, w, l, -rz - np.pi / 2
...
def corner_to_center_box3d(boxes_corner, coordinate='camera', T_VELO_2_CAM=None, R_RECT_0=None):
    ...
    for roi in boxes_corner:
        if cfgp.CORNER2CENTER_AVG:  # average version
            roi = np.array(roi)
...
def lidar_to_bird_view_img(lidar, factor=1):
    ...
    #   birdview: (w, l, 3)
    birdview = np.zeros(
        (cfgp.INPUT_HEIGHT * factor, cfgp.INPUT_WIDTH * factor, 1))
    for point in lidar:
        x, y = point[0:2]
        if cfgp.X_MIN < x < cfgp.X_MAX and cfgp.Y_MIN < y < cfgp.Y_MAX:
            x, y = int((x - cfgp.X_MIN) / cfgp.VOXEL_X_SIZE *
                       factor), int((y - cfgp.Y_MIN) / cfgp.VOXEL_Y_SIZE * factor)
            birdview[y, x] += 1
    ...
    if batch_score:
        ...
        for boxes, scores, clses in zip(batch_box3d, batch_score, batch_cls):
            ...
            for box, score, cls in zip(boxes, scores, clses):
                if coordinate == 'camera':
                    box3d = box
                    box2d = lidar_box3d_to_camera_box(camera_to_lidar_box(box[np.newaxis, :].astype(np.float32), T_VELO_2_CAM, R_RECT_0), cal_projection=False, P2=P2, T_VELO_2_CAM=T_VELO_2_CAM, R_RECT_0=R_RECT_0)[0]
                else:
                    box3d = lidar_to_camera_box(box[np.newaxis, :].astype(np.float32), T_VELO_2_CAM, R_RECT_0)[0]
                    box2d = lidar_box3d_to_camera_box(box[np.newaxis, :].astype(np.float32), cal_projection=False, P2=P2, T_VELO_2_CAM=T_VELO_2_CAM, R_RECT_0=R_RECT_0)[0]
                x, y, z, h, w, l, r = box3d
                box3d = [h, w, l, x, y, z, r]
                #label.append(template.format(cls, 0, 0, 0, *box2d, *box3d, float(score)))
                #label.append(template.format(cls, -1, -1, -1, *box2d, *box3d, float(score)))
                if cls == 1.0:
                    ss = 'Car'
                elif cls == 0.0:
                    ss = 'Pedestrian'
                    ...
                    ss = 'Misc'
                alpha = compute_alpha(x, z, r)

                label.append(ss + " " + template.format(-1, -1, alpha, *box2d, *box3d, float(score)))
        ...
        for boxes, clses in zip(batch_box3d, batch_cls):
            ...
            for box, cls in zip(boxes, clses):
                ...
                #label.append(template.format(cls, -1, -1, -1, *box2d, *box3d))
                label.append(ss + ' ' + template.format(-1, -1, alpha, *box2d, *box3d))
            batch_label.append(label)
...
def cal_anchors():
    ...
    x = np.linspace(cfgp.X_MIN, cfgp.X_MAX, cfgp.FEATURE_WIDTH)
    y = np.linspace(cfgp.Y_MIN, cfgp.Y_MAX, cfgp.FEATURE_HEIGHT)
    cx, cy = np.meshgrid(x, y)
    # all is (w, l, 2)
    cx = np.tile(cx[..., np.newaxis], 2)
    cy = np.tile(cy[..., np.newaxis], 2)
    cz = np.ones_like(cx) * cfgp.ANCHOR_Z
    w = np.ones_like(cx) * cfgp.ANCHOR_W
    l = np.ones_like(cx) * cfgp.ANCHOR_L
    h = np.ones_like(cx) * cfgp.ANCHOR_H
    r = np.ones_like(cx)
    r[..., 0] = 0  # 0
    r[..., 1] = 90 / 180 * np.pi  # 90
...
def cal_rpn_target(labels, feature_map_shape, anchors, cls='Car', coordinate='lidar'):
    ...
    for batch_id in range(batch_size):
        ...
        # find anchor iou > cfgp.XXX_POS_IOU
        id_pos, id_pos_gt = np.where(iou > cfgp.RPN_POS_IOU)

        # find anchor iou < cfgp.XXX_NEG_IOU
        id_neg = np.where(np.sum(iou < cfgp.RPN_NEG_IOU,
        ...
        targets[batch_id, index_x, index_y, np.array(index_z) * 7 + 2] = (
            batch_gt_boxes3d[batch_id][id_pos_gt, 2] - anchors_reshaped[id_pos, 2]) / cfgp.ANCHOR_H
...
def delta_to_boxes3d(deltas, anchors, coordinate='lidar'):
    ...
    boxes3d[..., [2]] = deltas[..., [2]] * \
        cfgp.ANCHOR_H + anchors_reshaped[..., [2]]
...
def cal_iou2d(box1, box2, T_VELO_2_CAM=None, R_RECT_0=None):
    ...
    # Output :
    #   iou
    buf1 = np.zeros((cfgp.INPUT_HEIGHT, cfgp.INPUT_WIDTH, 3))
    buf2 = np.zeros((cfgp.INPUT_HEIGHT, cfgp.INPUT_WIDTH, 3))
